...
public class Inventory : MonoBehaviour {
	...
	List<InventorySlot> occupiedInventorySlots = new List<InventorySlot>();

	void Start()
	{
		for(int i = 0; i < inventorySlotsUsed; i++)
		{
	...
	{
		InventorySlot newSlot = Instantiate(inventorySlotPrefab, inventoryContentArea);
		newSlot.item = slotItem;
		newSlot.stackSize = slotItem.stackSize;
		newSlot.UpdateUI();
